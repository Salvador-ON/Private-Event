...
RSpec.describe 'index page', type: :feature do
  before(:each) do
    ...
    @user2.save
    @event1 = Event.new(description: 'felipe party', location: 'salta', user_id: @user1.id, event_date: Date.new(2001,2,3))
    @event1.save
    @event2 = Event.new(description: 'juanito event 2', location: 'salta', user_id: @user1.id, event_date: Date.new(2021,2,3))
    @event2.save
  ...
  scenario 'login with user1 should show event1 description' do
    ...
    click_button "Submit"
    page.should have_content("#{@event1.description}")
  end
  scenario 'login with user1 should not show event2 description' do
    visit '/login'
    fill_in "id", :with => @user2.id
    click_button "Submit"
    page.should have_no_content("#{@event2.description}")
  end
  scenario 'login with user1 and watch upcomming events' do
    ...
    page.should have_content("#{@event2.description}")
  end
  scenario 'login with user1 and watch upcomming events, should not appear event1' do
    ...
    click_button "Submit"
    visit '/?type=upcomming'
    page.should have_no_content("#{@event1.description}")
  end
